...
export function SportComparisonChart() {
  ...
  const totalMembers = chartData.reduce((sum, item) => sum + item.payingMembers, 0)
  ...
  return (
    <Card>
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-2 sm:flex-row">
        ...
        <Select value={activeMetric} onValueChange={(value) => setActiveMetric(value as MetricType)}>
          <SelectTrigger
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <div className="grid gap-4 md:grid-cols-2 mb-6">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <div className="text-2xl font-bold">{formatValue(totalRevenue, "revenue")}</div>
            <div className="text-xs text-muted-foreground">
            ...
            <YAxis
              ...
              tickFormatter={(value) => {
                if (activeMetric === "revenue") {
                  return value >= 1000 ? `RM${(value / 1000).toFixed(1)}k` : `RM${value}`
                }
                ...
                <ChartTooltipContent
                  ...
                  formatter={(value, name, payload) => {
                    ...
                    return [
                      ...
                      <span className="text-muted-foreground text-xs" key={`${name}-label`}>
                        {activeMetric === "payingMembers" ? "Paying Members" : "Revenue"}
                      </span>
                    ]
                  }}
                  indicator="dot"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6 pt-6 border-t">
          {chartData.map((sport, index) => (
            <div key={sport.sport} className="flex flex-col space-y-2 p-4 rounded-lg border bg-muted/50">
              <div className="flex items-center space-x-2">
                <div 
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{sport.payingMembers.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Revenue</p>
                  <p className="font-semibold">{formatValue(sport.revenue, "revenue")}</p>
                </div>
              </div>
              <div className="text-xs text-muted-foreground">
                {((sport.payingMembers / totalMembers) * 100).toFixed(1)}% of total members
              </div>
            </div>
          ))}
        </div>
