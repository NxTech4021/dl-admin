...
interface PlayerProfileData {
  ...
  skillRatings: Record<string, { rating: number; confidence: string; rd: number }> | null
  accounts: { providerId: string; createdAt: string }[]
  sessions: { ipAddress: string | null; userAgent: string | null; expiresAt: string }[]
}
...
export function PlayerProfile({ playerId }: PlayerProfileProps) {
  ...
  const formatDate = (dateString: string) => new Date(dateString).toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' })
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <Badge variant={profile.status === 'active' ? 'default' : 'secondary'} className="capitalize mt-1">
                      {profile.status}
                    </Badge>
                  </div>
                </div>
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span className="capitalize">{profile.gender || 'Gender not set'}</span>
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <CardContent>
                {profile.skillRatings ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {Object.entries(profile.skillRatings).map(([sport, rating]) => (
                      <div key={sport} className="rounded-lg border p-3 text-center">
                        <p className="text-xs uppercase text-muted-foreground">{sport}</p>
                        <p className="text-2xl font-bold">{rating.rating.toFixed(1)}</p>
                        <Badge variant="outline" className="capitalize text-xs">{rating.confidence}</Badge>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">No skill ratings available.</p>
                )}
              </CardContent>
            </Card>
                ...
                <Table>
                  ...
                  <TableBody>
                    {profile.questionnaires.map((q, index) => (
                      <TableRow key={index}>
                        <TableCell className="capitalize font-medium">{q.sport}</TableCell>
                        <TableCell>{q.completedAt ? formatDate(q.completedAt) : 'Incomplete'}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right font-mono">
                          {q.result?.rating ? (q.result.rating / 1000).toFixed(1) : 'N/A'}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
              ...
              <Table>
                ...
                <TableBody>
                  {profile.accounts.map((acc, i) => (
                    <TableRow key={i}>
                      <TableCell className="capitalize font-medium">{acc.providerId}</TableCell>
               ...
               <Table>
                 ...
                 <TableBody>
                   {profile.sessions.map((sess, i) => (
                     <TableRow key={i}>
                       <TableCell className="text-xs text-muted-foreground">{sess.userAgent}</TableCell>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {profile.questionnaires.length > 0 ? (
              ...
              <Collapsible key={index} className="rounded-lg border">
                <CollapsibleTrigger className="flex w-full items-center justify-between p-4 text-left">
                  <div className="font-medium">
                    <span className="capitalize">{q.sport}</span> Questionnaire
                    <span className="ml-4 text-sm text-muted-foreground">
                      {q.completedAt ? `(Completed on ${formatDate(q.completedAt)})` : '(Incomplete)'}
                    </span>
                  </div>
              ...
              <p className="text-sm text-muted-foreground p-4">No questionnaire data found for this player.</p>
            )}
          </CardContent>
        </Card>
      </TabsContent>
